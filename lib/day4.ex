defmodule Day4 do
  ...
  def parse_input do
    {:ok, input} = :code.priv_dir(:aoc2015elixir) |> Path.join("day4.txt") |> File.read
    String.trim(input)
  end

  def md5_pair(secret, n) do
    {n, :crypto.hash(:md5, secret <> to_string(n)) |> Base.encode16}
  end

  def hash_number_leading_zeros(secret, n) do
    zeros = String.duplicate("0", n)
    Stream.iterate(0, &(&1 + 1))
    |> Stream.map(&(md5_pair(secret, &1)))
    |> Stream.filter(fn {_, hash} -> String.starts_with?(hash, zeros) end)
    |> Stream.take(1)
    |> Enum.to_list
    |> hd
    |> elem(0)
