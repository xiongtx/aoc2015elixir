defmodule Day8 do
  ...
  def total_original_escaped_length(strings) do
    strings
    |> Enum.map(&(String.length(&1) - escaped_length(&1)))
    |> Enum.sum
  end

  def encoded_length(s) do
  ...
  def total_encoded_original_length(strings) do
    strings
    |> Enum.map(&(encoded_length(&1) - String.length(&1)))
    |> Enum.sum
  end
end
