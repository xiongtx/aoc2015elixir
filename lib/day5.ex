defmodule Day5 do
  ...
  def parse_input do
    {:ok, input} = :code.priv_dir(:aoc2015elixir) |> Path.join("day5.txt") |> File.read
    input |> String.split
  end

  def nice?(s) do
    Regex.run(~r/[aeiou].*[aeiou].*[aeiou]/, s)
    && Regex.run(~r/(.)\1/, s)
    && !Regex.run(~r/ab|cd|pq|xy/, s)
  end

  def number_nice(strings) do
    strings |> Enum.filter(&nice?/1) |> length
  end

  def new_nice?(s) do
    Regex.run(~r/(..).*\1/, s) && Regex.match?(~r/(.).\1/, s)
  end

  def new_number_nice(strings) do
    strings |> Enum.filter(&new_nice?/1) |> length
  end
end
