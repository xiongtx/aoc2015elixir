defmodule Day2 do
  ...
  2*h*l. The elves also need a little extra paper for each present: the area
  ...
  def parse_input do
    {:ok, input} = :code.priv_dir(:aoc2015elixir) |> Path.join("day2.txt") |> File.read
    input
    |> String.split
    |> Enum.map(&(Regex.split(~r{x}, &1)))
  ...
  def ribbon_area([x,y,z]) do
    2*x + 2*y + x*y*z
  end

  def total_ribbon_area do
    parse_input()
    |> Enum.map(&ribbon_area/1)
    |> Enum.sum
  end
